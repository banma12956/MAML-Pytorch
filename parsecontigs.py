...
_KERNEL = utils.read_npz(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              "kernel.npz"))
...
def read_contigs(filehandle, minlength=100):
    ...
    raw = utils.PushArray(np.float32)
    projected = utils.PushArray(np.float32)
    lengths = utils.PushArray(np.int)
    contignames = list()

    entries = utils.byte_iterfasta(filehandle)
